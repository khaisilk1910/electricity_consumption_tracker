...
async def async_setup_entry(hass, entry):
    ...
    # Đăng ký Device Registry để gom nhóm sensor
    ...
    def init_db():
        """Khởi tạo database giống hệt cấu trúc file đính kèm."""
    ...
    def calculate_cost(kwh, year, month):
        """Tính tiền theo biểu giá lũy tiến lịch sử."""
    ...
    async def update_data(now=None):
        """Tự động quét chỉ số từ sensor nguồn."""
        source = entry.data[CONF_SOURCE_SENSOR]
        state = hass.states.get(source)
        val = 0.0
        if not state or state.state in ["unknown", "unavailable"]:
            async_create(hass, title="Lỗi Sensor", message=f"Sensor `{source}` không có dữ liệu. Tự gán = 0.", notification_id=f"err_{entry.entry_id}")
        else:
        ...
        # Ép buộc sensor cập nhật lại trạng thái ngay lập tức
    ...
    # Kích hoạt quét dữ liệu lần đầu ngay khi setup thành công
